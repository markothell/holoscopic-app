...
import { HoloscopicActivity as HoloscopicActivityType, Rating, Comment } from '@/models/Activity';
...
export default function ResolveActivity({ activity: initialActivity, sequenceId }: ResolveActivityProps) {
  ...
  const [mobileResultsTab, setMobileResultsTab] = useState<'map' | 'comments'>('map');
  ...
  const navigateToScreen = (screenIndex: number) => {
    const screens = ['', 'object-name-screen', 'quadrant-screen', 'comment-screen', 'results-screen'];
  ...
  useEffect(() => {
    ...
    if (dbObjectName) {
      ...
      localStorage.setItem(`objectName_${activity.id}_slot${currentSlot}`, dbObjectName);
    } else {
      // Fall back to localStorage
      const storedObjectName = localStorage.getItem(`objectName_${activity.id}_slot${currentSlot}`);
      if (storedObjectName) {
  ...
  // Get activityId from the activity prop
  const activityId = activity?.id || initialActivity.id;
  ...
  const handleRatingSubmit = async (position: { x: number; y: number; quadrant?: number }) => {
  ...
  // Activity not found (shouldn't happen since activity is passed as prop)
  if (!activity) {
    return (
      <div className="min-h-screen bg-[#1A1714] flex items-center justify-center p-4">
  ...
  return (
    <div className="min-h-screen bg-[#1A1714]" style={{ minHeight: '-webkit-fill-available' }}>
      ...
      <div ref={swipeRef} className="relative touch-pan-y">
                ...
                <button
                  onClick={() => {
                    if (activity.status === 'completed') {
                      ...
                      navigateToScreen(4); // Non-creators go straight to results
                    }
                  }}
        ...
        {/* Screen 3: Quadrant Selector */}
        <div id="quadrant-screen" className="min-h-screen flex flex-col bg-[#0a0f1a] text-white relative">

          <div className="flex flex-col items-center justify-center flex-1 w-full max-w-4xl mx-auto px-4 pb-24">
            <QuadrantSelector
              activity={activity}
              onQuadrantSelect={handleRatingSubmit}
        ...
        <div id="results-screen" className="min-h-screen lg:h-screen bg-[#0a0f1a]">
          ...
          <div className="sm:hidden flex-1 flex flex-col pt-4 pb-4">
            <div className="flex-shrink-0 px-4 mb-3">
              ...
              <div className="ml-16">
                <p className="text-sm text-gray-300 mb-1">Step 4</p>
            ...
            <div className="flex-shrink-0 flex justify-center mb-2 px-4">
              <div className="flex bg-[#111827] rounded-lg p-1 border border-white/10">
                <button
                  onClick={() => setMobileResultsTab('map')}
                  className={`px-4 py-2 rounded-md transition-colors ${
                    mobileResultsTab === 'map' ? 'bg-[#C83B50] text-white' : 'text-[#A89F96] hover:text-white'
                  }`}
                ...
                <button
                  onClick={() => setMobileResultsTab('comments')}
                  className={`px-4 py-2 rounded-md transition-colors ${
                    mobileResultsTab === 'comments' ? 'bg-[#C83B50] text-white' : 'text-[#A89F96] hover:text-white'
                  }`}
                  style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.62rem', letterSpacing: '0.15em', textTransform: 'uppercase' }}
                >
                  Comments
          ...
          <div className="hidden sm:flex sm:h-full lg:h-full" ref={resultsRef}>
            ...
            <div className="flex-1 flex flex-col px-8 py-6">
              ...
              {/* Tab bar for sm-lg range (hidden at lg+) */}
              ...
              {/* Results View - full width (always visible at lg+, conditional at sm-lg) */}
              <div className={`flex-1 flex items-center justify-center ${mobileResultsTab === 'comments' ? 'lg:flex hidden' : 'flex'}`}>
              ...
              {/* Comments tab content for sm-lg range */}
              {mobileResultsTab === 'comments' && (
