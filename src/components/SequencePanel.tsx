...
export default function SequencePanel({
  ...
  const [activities, setActivities] = useState<Array<{activityId: string; order: number; autoClose: boolean; duration: number | null; openedAt: Date | string | null; closedAt: Date | string | null; parentActivityIds: string[]; round: number | null; openOnCreate: boolean}>>([]);
  ...
  const [showActivityModalBottom, setShowActivityModalBottom] = useState(false);
  ...
  const pendingCreateRefresh = useRef(false);
  ...
  // Passoff fix: use visibilitychange (reliable for tab switching) to detect return from "create new"
  useEffect(() => {
    const onVisibilityChange = async () => {
      if (document.visibilityState === 'visible' && pendingCreateRefresh.current) {
        pendingCreateRefresh.current = false;
        const refreshed = await refreshActivities();
        if (!refreshed) return;
        // Auto-add any activities that weren't in the list before the new tab was opened
        const newActivities = refreshed.filter(a => !prevActivityIdsRef.current.has(a.id));
        if (newActivities.length > 0) {
          setActivities(prev => {
            const maxOrder = prev.length > 0 ? Math.max(...prev.map(a => a.order)) : 0;
            const toAdd = newActivities.map((act, i) => ({
              activityId: act.id,
              order: maxOrder + i + 1,
              autoClose: false,
              duration: null,
  ...
  const handleAddActivity = (mode: 'existing' | 'clone' | 'create') => {
    ...
    if (mode === 'existing') {
      ...
      prevActivityIdsRef.current = new Set(availableActivities.map(a => a.id));
      pendingCreateRefresh.current = true;
  ...
  const handleActivityChange = (index: number, field: 'activityId' | 'order' | 'autoClose' | 'duration' | 'round' | 'openOnCreate', value: string | number | boolean | null) => {
    ...
    <div style={sx.card}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div>
          ...
          {activities.length === 0 && activityMode !== 'clone' ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {activities.map((activity, index) => {
                const isCollapsed = collapsedActivities.has(index);
                const actTitle = availableActivities.find(a => a.id === activity.activityId)?.title;
                return (
                <div key={index} style={sx.sectionCard}>
                  ...
                  <div
                    ...
                    onClick={() => toggleActivityCollapsed(index)}
                  >
                    <div style={{
                      flexShrink: 0,
                      width: '2rem',
                    ...
                    <span style={{ flex: 1, fontFamily: 'var(--font-cormorant), Georgia, serif', fontSize: '1rem', color: '#0F0D0B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {actTitle || (activity.activityId ? activity.activityId : 'Select an activity…')}
                    </span>
                    <span style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.7rem', color: '#6B6560', flexShrink: 0 }}>
                      {isCollapsed ? '+' : '−'}
                    </span>
                  </div>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                          <input
                            type="checkbox"
                            checked={activity.openOnCreate}
                            onChange={(e) => handleActivityChange(index, 'openOnCreate', e.target.checked)}
                            style={sx.checkbox}
                          />
                          <span style={{ marginLeft: '0.5rem', fontFamily: 'var(--font-cormorant), Georgia, serif', fontSize: '0.9rem', color: '#0F0D0B' }}>
                            Open when sequence starts
                          </span>
                        </label>

                        {/* Round (graph vertical level) */}
                        <div>
                          <label style={{ ...sx.hint, margin: 0, marginBottom: '0.25rem', display: 'block' }}>Round</label>
                          <input
                            type="number"
                            value={activity.round ?? ''}
                            onChange={(e) => handleActivityChange(index, 'round', e.target.value ? parseInt(e.target.value) : null)}
                            style={{ ...sx.input, width: '5rem' }}
                            min="1"
                            placeholder="—"
                          />
                          <p style={sx.hint}>Sets graph level (overrides parent links)</p>
                        </div>
          ...
          {/* Bottom add activity button — only shown when there are already activities */}
          {activities.length > 0 && activityMode !== 'clone' && (
            <div style={{ marginTop: '0.75rem', position: 'relative' }}>
              <button
                type="button"
                onClick={() => setShowActivityModalBottom(!showActivityModalBottom)}
                style={{ ...sx.secondaryBtn, width: '100%', padding: '0.5rem' }}
              >
                + Add Activity
              </button>
              {showActivityModalBottom && (
                <div style={{
                  position: 'absolute',
                  left: 0,
                  top: '100%',
                  ...
                  {(['existing', 'clone', 'create'] as const).map(mode => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => { setShowActivityModalBottom(false); handleAddActivity(mode); }}
                      style={{
                        width: '100%', textAlign: 'left', padding: '0.5rem 0.75rem',
                        fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.65rem',
                        letterSpacing: '0.08em', color: '#0F0D0B', background: 'none',
        ...
        {/* Actions */}
        <div style={{ display: 'flex', gap: '0.75rem', paddingTop: '1rem', borderTop: '1px solid #D9D4CC', flexWrap: 'wrap' }}>
          <button
            type="submit"
            disabled={loading}
            style={{
