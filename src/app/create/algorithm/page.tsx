...
function AlgorithmForm() {
  ...
  const [config, setConfig] = useState<{ algorithmPublishCost: number } | null>(null);
  ...
  useEffect(() => {
    apiFetch('/admin/config').then(d => setConfig({ algorithmPublishCost: d.holons?.algorithmPublishCost ?? 150 })).catch(() => {});
    if (forkFromId) {
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.5rem' }}>
        <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-dm-mono), monospace', color: 'var(--text-muted)' }}>
          Balance: {holonBalance ?? '—'} H · Costs {config?.algorithmPublishCost ?? '…'} H to publish
        </span>
