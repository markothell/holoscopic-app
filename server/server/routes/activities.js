...
router.post('/:id/rating', async (req, res) => {
  try {
    ...
    let participant = activity.participants.find(p => p.id === userId);
    if (!participant) {
      participant = {
        id: userId,
    ...
    const existingRatingIndex = activity.ratings.findIndex(r => r.userId === userId);
    const newRating = {
      userId,
    ...
    if (existingRatingIndex >= 0) {
      activity.ratings[existingRatingIndex] = newRating;
    } else {
