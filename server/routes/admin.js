...
router.post('/holons/award', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: 'User not found' });
    const balance = await transact({ userId: user.id, type: 'join_bonus', amount });
...
router.put('/config', async (req, res) => {
  try {
    ...
    const update = { holons, quorum, updatedAt: new Date() };
    if (topicsActivityId !== undefined) update.topicsActivityId = topicsActivityId || null;
