...
| **M6** | Launch — **the plumbing is done, the pass is not** | Accessibility + performance pass, `CLIENT_URL`, Vercel project, Beacon, `ensure-indexes.js` against production. | **Provisioned 2026-08-10**: indexes (below), the `circlemo` instance, `CLIENT_URL`, `THRESHOLD_URL`, the Vercel project on `threshold.holoscopic.io`, and a Threshold-only Blob store. `/health` reads `mediaBackup: ready` and the boot log carries `⏱️  Circle rounds every 60s`. **What remains is the pass itself** — accessibility and performance — plus the phone recording (M3b) and a real inbox (M4). |
| | ↳ **indexes before the first write** — **DONE** | Three of the new indexes are `unique` and therefore correctness, not speed. `ensure-indexes.js` **skips a collection that does not exist yet**, and it says so — so running it once against production before Threshold takes any traffic is the whole job, and running it after means building a unique index over rows that may already violate it. Order matters here in a way it does not for the performance indexes. **`scripts/init-threshold.js` is what does it**, because it materializes the three collections first; `ensure-indexes.js` alone would have skipped all three. | Ran against `live.ofmfipp` / `holoscopic-db` on 2026-08-10: 7 indexes present on each of `circles`, `thresholdshares`, `thresholdrankings`. `ensure-indexes.js` under `NODE_ENV=production` then reported every one `OK — already present`, `created: 0`, and no SKIP lines for the three. |
