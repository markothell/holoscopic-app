...
// The circle home map: the circle drawn as a circle.
//
// Members sit on a ring — all equal, all facing a common centre, which is the
// exact claim the /circles lander makes in prose. What the group has explored
// TOGETHER gathers in the middle: one node per finished topic, sized by how
// much of the circle told a story on it, positioned toward the members who
// did, in the threshold grey — it is what the group made, so it wears the
// colour of the line the group found. What a member explored ALONE — a topic
// only they told a story on — is a short spur pointing outward from them, per
// the sketch: an edge with no circle at its end.
//
// Everything here is render-time geometry over the snapshot plus
// `participation` (server-redacted, D9/D17) — the map can never leak an
// identity because it never receives one it may not show. The live topic
// appears at most as a count; the only edge it can carry is your own.
//
...
// circle scale (4–12 people, tens of topics), and a map that jiggles between
// visits would read as the group having moved when nobody has.
...
function layout(members: Member[], seeds: Seed[], participation: SeedParticipation[], liveSeedId: string | null, myUserId: string | null) {
  ...
  // dead band before the caption. Width stays full so the ring stays centred.
...
export function CircleMap({ circle, userId }: { circle: Circle; userId: string | null }) {
  ...
  const liveHref = (seed: Seed) =>
    seed.phase === 'share' ? `${base}/share` : seed.phase === 'rank' ? `${base}/rank` : base;
  ...
  return (
    <figure className="mx-auto my-2 w-full max-w-[26rem]">
      ...
      >
        ...
        {/* Solo explorations: a short edge with no circle at its end. */}
        {geo.spurs.map(spur => (
          <a
            key={spur.seed.id}
            href={`${base}/cycle/${spur.seed.id}`}
          ...
          >
            ...
            <line
              ...
              stroke="var(--threshold)" strokeWidth="2.5" strokeLinecap="round"
            />
            <line
              x1={spur.x1} y1={spur.y1} x2={spur.x2} y2={spur.y2}
        ...
        {/* Shared explorations: sized by how much of the circle told a story. */}
        {geo.nodes.map(node => (
          <a
            key={node.seed.id}
            href={node.live ? liveHref(node.seed) : `${base}/cycle/${node.seed.id}`}
            aria-label={
              node.live
          ...
          >
            ...
            <circle
              ...
              fill={node.live ? 'var(--card)' : 'var(--threshold-soft)'}
              stroke={node.live ? 'var(--ink)' : 'var(--threshold)'}
              strokeWidth={node.live ? 1.6 : 1.2}
            />
          </a>
        ))}
            ...
            <g key={m.userId} className="cm-fade">
              ...
              {m.pictureUrl ? (
                ...
                <text
                  ...
                  textAnchor="middle"
                  fill="var(--ink-soft)"
                  style={{ font: `${Math.round(geo.memberR * 0.78)}px var(--font-voice)` }}
                >
                  {initials(m.username)}
                </text>
              )}
