...
//
// THIS FILE IS MIRRORED, byte for byte apart from its `App` union, in
// apps/chorus, apps/spectrum, apps/synthesis and apps/holoscopic-game. It is
// not in a package because the apps that need it do not share one: chorus,
// spectrum, threshold and platform have no dependency on @hs/activities, and
// giving a memorial app the whole activity engine to import forty lines of
// fetch would be a worse trade than five copies of a leaf file that changes
// approximately never. If you change the wire shape here, change it in all
// five — the server validates `app` against an allowlist (utils/traffic.js#APPS),
// so a drifted copy fails loudly as a dropped event rather than quietly as a
// wrong number.
//
...
export default function Beacon({ app, instanceId, slug, trackClicks = false }: Props) {
  ...
  useEffect(() => {
    ...
    send({ app, type: 'view', path: pathname, instanceId, slug });
