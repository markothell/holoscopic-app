...
// One cycle's reveal (PLAN.md §6.3, D23/D24) — the whole surface, lifted out of
// `app/t/[urlName]/cycle/[seedId]/page.tsx` so the public demo renders THIS and
// not a copy of it. The page keeps the session, the fetch and the error states;
// everything a reader looks at is here, and both callers hand it the same three
// objects.
//
...
//
// A story is a dot with a short preview, expanding on tap; playback lives inside
// the expanded state, never on every dot.
//
...
//
// THE THRESHOLD APPEARS ONCE THREE PEOPLE HAVE SUBMITTED. Below that this
...
export function CycleReveal({ seed, shares, result, base, intro }: {
  seed: Seed;
  shares: Share[];
  result: SeedResult;
  /** Where "back to the circle" goes. `/t/<urlName>` for a member, `/demo` for
   *  the sample — the surface is otherwise identical. */
  base: string;
  /** Rendered at the top of the page, inside its column. The demo puts its
   *  label and its way out here; the real page passes nothing. */
      ...
      {enough ? (
        ...
        <>
          ...
          <ul className="mt-6 space-y-3">
            {result.shares.map(row => {
              const share = shares.find(s => s.id === row.shareId);
 ...
 * whole story, its teller and the split live — and where the player goes.
 * Putting playback on every dot would make a wall of controls out of a screen
 * whose subject is a shape.
 */
