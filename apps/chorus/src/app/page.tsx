...
// The landing page: who this is for, then everything people have left.
//
...
export default async function Home({
  ...
  try {
    [config, wall] = await Promise.all([
      memorialApi.config(),
      memorialApi.wall({ limit: 20, tags, sort }),
    ]);
  } catch {
  ...
  return (
    <main className="mx-auto max-w-md px-5 pb-24">
          ...
          <p className="voice py-10 text-center text-[1.125rem] leading-[1.6] text-ink-soft">
            {tags.length ? (
              <>
                ...
                <br />
                or <Link href="/" className="text-dial underline underline-offset-4">show all memories</Link>.
              </>
            ) : (
              <>
      ...
      <TagPortrait tags={config.tags.role} subjectName={name} params={params} />
    </main>
  );
}
