# Synthesis — the Union (∪) (spec, draft v0.2)

> **The Union** is the LLM's read of an idea's whole published corpus: everything
> the group has said, taken together. It is the raw material a member edits into a
> **statement** and puts to the group (see `MECHANISM.md`); it carries no votes and
> settles nothing on its own.
>
> **Replaces** the M3 "Ask the Group" open Q&A with a cached,
> whole-corpus statement of where the group currently stands — drawn from the
...
Reuse the embedding store (`synthesisembeddings`, `instanceId`-scoped, published-only)
...
- Pull the replies' `Entry.position` (`{x,y}∈[0,1]`) and the post's axes
  (`node.axisFrameIds` → `SynFrame` pole labels; poleA = the "most" end — right
...
New `utils/synUnion.js`, sibling to `utils/synthesisChat.js`:
...
Synthesis runs on **`claude-sonnet-5`** via a dedicated `SYN_UNION_MODEL`
...
- New `models/SynUnion.js`: one doc per `instanceId`, holding `corpusVersion`
  and a `brief` and `full` sub-artifact each `{ text, citations, generatedAt, model,
  atCorpusVersion }`.
- **Staleness:** `utils/synIndexHooks.js` (already firing on
...
- **Routes:**
  - `GET /synthesis/synthesis` → the cached `{ brief?, full?, corpusVersion, stale }`
    (may be empty/absent) — for showing the last result + a stale badge on open.
  - `POST /synthesis/synthesis { depth }` → (re)generate that depth, update cache,
...
3. **Output** — **both**: brief 2–4 sentences by default, a button for the full
   structured read.
4. **Model** — **upgrade to Sonnet** (`SYN_UNION_MODEL=claude-sonnet-5`).
5. **Positions** — **included**, as per-post/per-spectrum summaries (§5).

## 12. Backend surface (files)

- `models/SynUnion.js` (new) — per-instance cache + `corpusVersion`.
- `utils/synUnion.js` (new) — `selectCorpus`, `computePositional`,
  `prepareSynthesis`, cache read/write, `markStale(instanceId)`.
- `utils/synIndexHooks.js` (edit) — also `markStale` on corpus change.
- `routes/synthesis.js` (edit) — `GET /synthesis`, `POST /synthesis`.
...
- **Build:** `prepareSynthesis` + the two prompts, `selectCorpus`,
  `computePositional`, `SynUnion` cache + `markStale`, the two routes, the
