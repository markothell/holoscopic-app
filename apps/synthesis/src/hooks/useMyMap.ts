...
// and *also* fires the matching SynthesisService call in the background; a
...
// SynNode.find({instanceId, ownerId}) scan (GET /synthesis/nodes) — a thin
...
function toAxisSpecs(
  axisFrameIds: string[],
  seedMock: boolean,
  resolveLocalFrame?: (id: string) => Pick<SynFrame, 'poleA' | 'poleB'> | undefined,
): FrameSpec[] {
  return axisFrameIds.reduce<FrameSpec[]>((specs, frameId) => {
    ...
    if (local) {
      ...
      console.debug('[synthesis] could not resolve locally-coined frame', frameId);
    }
    return specs;
  }, []);
}
...
export function useMyMap(instanceId: string, userId: string, ownerHandle: string, options: {
  ...
  const [nodes, setNodes] = useState<SynNode[]>(seedMock ? MOCK_NODES : []);
  ...
  useEffect(() => {
    if (seedMock) return;
    let cancelled = false;
    SynthesisService.myMap(instanceId, userId)
      .then(({ nodes: serverNodes }) => {
        if (!cancelled) setNodes(serverNodes);
      })
      .catch(err => console.debug('[synthesis] map load failed', err));
    return () => {
      cancelled = true;
    };
  ...
  const mergeNode = useCallback((node: SynNode) => {
    setNodes(prev => {
  ...
  function sync(promise: Promise<{ node: SynNode }>, localId: string, touchesAxes: boolean) {
    promise
      ...
      .catch(err => {
        ...
        console.debug('[synthesis] backend not synced for', localId, err);
      });
  }
  ...
  const addChild = useCallback((
    ...
    setError(null);
    sync(
      SynthesisService.createChild(instanceId, parentId, kind, content, toAxisSpecs(axisFrameIds, seedMock, resolveLocalFrame), userId),
      id,
      axisFrameIds.length > 0,
    );
    return draft;
  ...
  const marry = useCallback((
    ...
    setError(null);
    sync(
      SynthesisService.marry(instanceId, parentIds, kind, content, toAxisSpecs(axisFrameIds, seedMock, resolveLocalFrame), userId),
      id,
      axisFrameIds.length > 0,
    );
    return draft;
  ...
  // is the one content-edit path (mirrors utils/synNodes.js#editContent,
  ...
  const editNode = useCallback((nodeId: string, content: Partial<NodeContent>) => {
    ...
    SynthesisService.editContent(instanceId, nodeId, content, userId)
      .then(({ node: serverNode }) => {
      ...
      .catch(err => console.debug('[synthesis] edit not synced', err));
  }, [instanceId, userId]);
  ...
  const setAxes = useCallback((nodeId: string, axisFrameIds: string[]) => {
    ...
    setError(null);
    SynthesisService.setAxes(instanceId, nodeId, toAxisSpecs(axisFrameIds, seedMock, resolveLocalFrame), userId)
      .then(({ node: serverNode }) => {
