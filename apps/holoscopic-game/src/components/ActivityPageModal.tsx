...
import { CommentSection } from '@hs/activities';
import PreambleModal from './modals/PreambleModal';
import EntryModal from './modals/EntryModal';
import { normalizeActivityType, getActivityTypeConfig, REGISTRY } from '@hs/activities';
...
export default function ActivityPageModal({ activityId, sequenceId }: ActivityPageModalProps) {
  ...
  const [currentSnapshotQuestion, setCurrentSnapshotQuestion] = useState<SnapshotQuestion | null>(null);
  const [hoveredSlot, setHoveredSlot] = useState<number | null>(null);
  const [hasJoined, setHasJoined] = useState(false);
  const [activeCellFilteredIds, setActiveCellFilteredIds] = useState<string[] | null>(null);
  ...
  // Load activity — runs for authenticated and anonymous users alike
  useEffect(() => {
    if (authLoading) return;
  ...
  const handleEntrySubmit = async (data: {
    ...
    comment: string;
    snapshotAnswers?: import('@/models/Activity').SnapshotAnswer[];
  }) => {
    if (!activity || !userId) return;

    try {
      if (data.snapshotAnswers && data.snapshotAnswers.length > 0) {
  ...
  const handleSnapshotQuestionClick = (question: SnapshotQuestion) => {
    setCurrentSnapshotQuestion(question);
    setCurrentSlot(question.order); // each question gets its own slot so delete is scoped correctly
    if (!hasJoined) {
      setShowPreamble(true);
    } else {
      setShowEntryModal(true);
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[#1A1714]">
              ...
              {actType === 'snapshot' ? (
                ...
                <>
                  ...
                  <div className="flex gap-3 flex-wrap mt-2">
                    {[...(activity.snapshotQuestions || [])].sort((a, b) => a.order - b.order).map(q => {
                      const qData = getSnapshotQuestionData(q.id);
                      return (
                        <button
                          key={q.id}
                          onClick={() => handleSnapshotQuestionClick(q)}
                          className="flex flex-col items-center gap-1 transition-opacity hover:opacity-80"
                          aria-label={q.topic || q.label}
                        >
                          <div
            ...
            <div className="flex-1 min-h-0">
              {resolveTab === 'map' ? (
                <TypeResults
                  ...
                  onActiveCellChange={setActiveCellFilteredIds}
                />
              ) : (
      ...
      <EntryModal
        ...
        onClose={() => { setShowEntryModal(false); setCurrentSnapshotQuestion(null); }}
        ...
        existingData={currentSnapshotQuestion
          ? getSnapshotQuestionData(currentSnapshotQuestion.id)
          : getSlotData(currentSlot)}
        snapshotQuestion={currentSnapshotQuestion ?? undefined}
      />
    </div>
  );
}
