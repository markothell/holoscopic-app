...
export class ActivityService {
  ...
  static async createActivity(formData: ActivityFormData): Promise<HoloscopicActivity> {
    try {
      ...
      if (formData.activityType === 'snapshot') {
        (activityData as any).snapshotQuestions = formData.snapshotQuestions || [];
        (activityData as any).xAxisPoints = formData.xAxisPoints || 2;
        (activityData as any).yAxisPoints = formData.yAxisPoints || 2;
        (activityData as any).xAxisLabels = formData.xAxisLabels || [];
        (activityData as any).yAxisLabels = formData.yAxisLabels || [];
      }
  ...
  static async submitRating(activityId: string, userId: string, position: { x: number; y: number }, objectName?: string, slotNumber: number = 1, questionId?: string | null): Promise<Rating> {
    // Add delay to prevent rapid successive calls
    await new Promise(resolve => setTimeout(resolve, 100));

    try {
      const response = await authFetch(`${API_BASE_URL}/activities/${activityId}/rating`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  static async submitComment(activityId: string, userId: string, text: string, objectName?: string, slotNumber: number = 1, questionId?: string | null): Promise<Comment> {
    try {
      const response = await authFetch(`${API_BASE_URL}/activities/${activityId}/comment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  static async voteComment(activityId: string, commentId: string, userId: string): Promise<Comment> {
    try {
      const response = await authFetch(`${API_BASE_URL}/activities/${activityId}/comment/${commentId}/vote`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
