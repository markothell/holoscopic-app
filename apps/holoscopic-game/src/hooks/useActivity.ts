...
interface UseActivityResult {
  activity: HoloscopicActivity | null;
  loading: boolean;
  error: string | null;
  refetch: () => Promise<void>;
  updateActivity: (updates: Partial<HoloscopicActivity>) => void;
...
export function useActivity(activityId: string): UseActivityResult {
  ...
  const updateActivity = useCallback((updates: Partial<HoloscopicActivity>) => {
    setActivity(prev => {
      if (!prev) return null;
      return { ...prev, ...updates };
    });
