...
function PatternForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (forkFromId) {
        const { algorithm: alg, newSequenceUrlName } = await PatternService.fork(userId, forkFromId, payload);
        refreshBalance();
        const suffix = newSequenceUrlName ? '?sequenceCloned=1' : '';
        router.push(`/patterns/${alg.id}${suffix}`);
      } else {
        const alg = await PatternService.publish(userId, payload);
        refreshBalance();
        router.push(`/patterns/${alg.id}`);
      }
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ padding: '3rem', textAlign: 'center' }}>
        <p style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }}>You need to be signed in to publish an pattern.</p>
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.5rem' }}>
        ...
        <button type="submit" disabled={submitting}
          ...
          {submitting ? 'Publishing…' : forkFromId ? 'Publish Fork' : 'Publish Pattern'}
        </button>
      </div>
    </form>
  );
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      <header style={{ borderBottom: '1px solid var(--border-subtle)', padding: '0 1.5rem' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <Link href="/patterns" style={{ fontSize: '0.7rem', fontFamily: 'var(--font-dm-mono), monospace', letterSpacing: '0.1em', color: 'var(--text-muted)', textDecoration: 'none', textTransform: 'uppercase' }}>
              ← Patterns
            </Link>
          </div>
          <UserMenu />
      ...
      <main style={{ maxWidth: 640, margin: '0 auto', padding: '3rem 1.5rem' }}>
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 0.4rem 0' }}>Publish Pattern</h1>
