...
function SequenceAdminContent() {
  ...
  const handleDeleteSequence = async (sequence: Sequence) => {
    const activityCount = sequence.activities.length;
    const baseConfirm = `Are you sure you want to delete "${sequence.title}"?`;
    if (!confirm(baseConfirm)) return;

    let deleteActivities = false;
    if (activityCount > 0) {
      deleteActivities = confirm(
        `Also delete the ${activityCount} activit${activityCount === 1 ? 'y' : 'ies'} inside this sequence?\n\nOK = delete activities too\nCancel = keep activities`
      );
    }

    try {
      await SequenceService.deleteSequence(sequence.id, deleteActivities);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImporting(true);
    try {
      const result = await ImportService.importSequence(file, userId);
      const sequencesData = await SequenceService.getAdminSequences(userId);
      setSequences(sequencesData);
      alert(`Imported successfully: "${result.sequenceUrlName}" with ${result.activityCount} activities.`);
    } catch (err: unknown) {
          ...
          <div>
            <div className={styles.sectionHeader}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                <label className={styles.secondaryBtn} style={{ cursor: importing ? 'wait' : 'pointer', opacity: importing ? 0.6 : 1 }}>
                    ...
                    <div className={styles.actions}>
                      ...
                      {sequence.status === 'active' && (
                        <>
                          <span className={styles.actionDot}>&middot;</span>
                          <Link
                            href={`/sequence/${sequence.urlName}/manage`}
