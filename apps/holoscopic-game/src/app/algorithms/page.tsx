    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
            ...
            <nav style={{ display: 'flex', gap: '1rem' }}>
              {['Topics', 'Inquiry', 'Algorithms'].map(item => (
                <Link key={item} href={`/${item.toLowerCase()}`}
                  style={{ fontSize: '0.7rem', fontFamily: 'var(--font-dm-mono), monospace', letterSpacing: '0.1em', color: item === 'Algorithms' ? 'var(--accent)' : 'var(--text-muted)', textDecoration: 'none', textTransform: 'uppercase' }}>
                  {item}
                </Link>
              ))}
            </nav>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '3rem 1.5rem' }}>
                ...
                return (
                  <div key={p.id} style={{ background: 'var(--bg-secondary)', border: `1px solid ${isIn ? 'var(--accent)' : 'var(--border-default)'}`, borderRadius: 10, padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', marginBottom: '0.6rem' }}>
                      <div style={{ flex: 1 }}>
                        <Link href={`/algorithms/${p.algorithmId}`}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <span style={{ fontSize: '0.62rem', fontFamily: 'var(--font-dm-mono), monospace', color: 'var(--text-muted)', flexShrink: 0 }}>
                        {p.signups.length}/{p.quorumThreshold} · {hoursLeft}h left · {p.proposedByName}{isProposer ? ' (you)' : ''}
                      </span>
                    </div>
                  </div>
                );
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {mySessions.map(s => (
                <div key={s.proposalId} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)', borderRadius: 10, padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ flex: 1 }}>
                    <Link href={`/algorithms/${s.algorithmId}`}
                      style={{ fontSize: '0.65rem', fontFamily: 'var(--font-dm-mono), monospace', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--accent)', textDecoration: 'none' }}>
                      {s.algorithmTitle}
                    </Link>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem' }}>
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 0.3rem 0' }}>Algorithms</h1>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: 0 }}>Reusable conversation patterns published by practitioners</p>
          </div>
          {isAuthenticated && (
            <Link href="/create/algorithm"
        ...
        {!loading && algorithms.length === 0 && (
          <div style={{ textAlign: 'center', padding: '4rem 0' }}>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>No algorithms published yet.</p>
            {isAuthenticated && (
              <Link href="/create/algorithm" style={{ fontSize: '0.8rem', color: 'var(--accent)', textDecoration: 'none' }}>Be the first to publish →</Link>
            )}
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {algorithms.map(alg => (
            <Link key={alg.id} href={`/algorithms/${alg.id}`} style={{ display: 'block', textDecoration: 'none', background: 'var(--bg-secondary)', border: '1px solid var(--border-default)', borderRadius: 12, padding: '1.25rem 1.5rem', transition: 'border-color 0.15s' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.3rem' }}>
                    <h2 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>{alg.title}</h2>
                    {alg.forkDepth > 0 && (
                  ...
                  <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-dm-mono), monospace', color: 'var(--text-muted)' }}>
                    {alg.authorName} · {new Date(alg.publishedAt).toLocaleDateString()}
                  </span>
                </div>
              </div>
            </Link>
