...
// SynFrame — an OasFrame-shaped "spectrum": two poles a thought's readers
...
//
// Scoped to the community Instance (a Synthesis community = a child Instance),
// mirroring OasFrame's instance scoping. Unlike OasFrame there is no gameId:
// the community instance IS the dedupe boundary.
const synthesisFrameSchema = new mongoose.Schema({
  id: {
    type: String,
    required: true,
    unique: true,
  ...
  // The deployment this community belongs to (the `synthesis` parent instance) —
...
module.exports = mongoose.model('SynFrame', synthesisFrameSchema);
