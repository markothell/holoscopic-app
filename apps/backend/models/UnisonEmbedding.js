...
// UnisonEmbedding — the vector index for the collective "Ask the Group" LLM
...
// removal hook were ever missed. Enforced server-side in utils/unisonIndex.js.
//
...
// swap behind utils/unisonIndex.js, not a data-model change here.
const unisonEmbeddingSchema = new mongoose.Schema({
  id: {
    type: String,
    required: true,
    unique: true,
  ...
  // The citation target's id: the UnisonNode.id for a node, the Entry.id for a
...
unisonEmbeddingSchema.index({ instanceId: 1, kind: 1, refId: 1 }, { unique: true });
// Reindex/removal of everything hanging off a post.
unisonEmbeddingSchema.index({ instanceId: 1, nodeId: 1 });

module.exports = mongoose.model('UnisonEmbedding', unisonEmbeddingSchema);
