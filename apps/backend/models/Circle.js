...
const circleSchema = new mongoose.Schema({
  ...
  config: {
    ...
    // Below it an ask is NOMINATED: visible in its own list and on the map,
    // gathering support, but never opened by a free slot.
    //
    // NULL — the default — means derive it from the roster: a third of the
    // circle, floor 2, capped at the member count. utils/circles.js
    // #approvalsToStart is the one implementation and explains why. A number
    // here overrides with a flat count, which is what circle settings will
    // write when somebody wants one.
    approvalsToStart: { type: Number, default: null, min: 1, max: 50 },
