...
router.get('/url/:urlName', async (req, res) => {
  try {
    ...
    // Populate host name from creator
    let host = null;
    if (sequence.createdBy) {
      const hostUser = await User.findOne({ id: sequence.createdBy });
      if (hostUser) {
        host = { userId: sequence.createdBy, name: hostUser.name || 'Host' };
      }
    }

    // Always filter hidden rounds — participant page shows what participants see
    const hiddenRoundNums = new Set(
      sequence.rounds.filter(r => r.hidden).map(r => r.number)
    );
    const visibleActivities = hiddenRoundNums.size > 0
      ? activitiesWithDetails.filter(a => !hiddenRoundNums.has(a.round ?? 1))
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (updates.updates !== undefined) sequence.updates = updates.updates;
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (deleteActivities && sequence.activities.length > 0) {
      const activityIds = sequence.activities.map(a => a.activityId);
      await Activity.deleteMany({ id: { $in: activityIds } });
    }

    await Sequence.deleteOne({ id });
...
// Open an activity in the sequence
router.post('/:id/activities/:activityId/open', async (req, res) => {
  try {
    const { id, activityId } = req.params;
    const sequence = await Sequence.findOne({ id });
    if (!sequence) return res.status(404).json({ error: 'Sequence not found' });
    await sequence.openActivity(activityId);
    res.json(sequence);
  } catch (error) {
    console.error('Error opening activity:', error);
    res.status(400).json({ error: error.message || 'Failed to open activity' });
  }
});

// Schedule activity close
router.post('/:id/activities/:activityId/schedule', async (req, res) => {
  try {
    const { id, activityId } = req.params;
    const { closedAt } = req.body;
    if (!closedAt) return res.status(400).json({ error: 'closedAt is required' });
    const sequence = await Sequence.findOne({ id });
    if (!sequence) return res.status(404).json({ error: 'Sequence not found' });
    await sequence.scheduleActivityClose(activityId, closedAt);
    res.json(sequence);
  } catch (error) {
    console.error('Error scheduling activity close:', error);
    res.status(400).json({ error: error.message || 'Failed to schedule close' });
  }
});

// Set round visibility
router.patch('/:id/rounds/:roundNumber/visibility', async (req, res) => {
  try {
    const { id, roundNumber } = req.params;
    const { hidden } = req.body;
    const sequence = await Sequence.findOne({ id });
    if (!sequence) return res.status(404).json({ error: 'Sequence not found' });
    await sequence.setRoundVisibility(parseInt(roundNumber), !!hidden);
    res.json(sequence);
  } catch (error) {
    console.error('Error setting round visibility:', error);
    res.status(500).json({ error: 'Failed to set round visibility' });
  }
});

// Get per-member activity stats for a sequence
...
// Duplicate a sequence — clones all contained Activity documents
router.post('/:id/duplicate', async (req, res) => {
  try {
    const { id } = req.params;
    const original = await Sequence.findOne({ id });
    if (!original) return res.status(404).json({ error: 'Sequence not found' });

    const Activity = require('../models/Activity');

    // Find a unique sequence urlName
    let baseUrl = `${original.urlName}-copy`;
    let urlName = baseUrl;
    let counter = 2;
    while (await Sequence.findOne({ urlName })) {
    ...
    const duplicate = new Sequence({
      ...
      welcomePage: original.welcomePage,
      requireInvitation: original.requireInvitation,
      invitedEmails: [...original.invitedEmails],
      rounds: original.rounds.map(r => ({ number: r.number, hidden: r.hidden })),
      activities: original.activities
        .filter(a => activityIdMap[a.activityId])
        .map(a => ({
          activityId: activityIdMap[a.activityId],
          order: a.order,
          openOnCreate: a.openOnCreate,
          // Remap parentActivityIds to new cloned IDs
          parentActivityIds: (a.parentActivityIds || [])
            .map(pid => activityIdMap[pid])
            .filter(Boolean),
          round: a.round,
          openedAt: null,
          closedAt: null,
