...
| `models/Circle.js` | The generic cohort + round machine (members, seed queue, phase machine) — activity-agnostic by design. Since 2026-08-17: `seeds[].activity` (per-seed module, null = the circle's own) and `config.maxLive` (concurrent cycles, default 1 = the original machine). Since 2026-09-16: **`hostId`, the movable host seat** — a user id holds it, `''` is deliberately VACATED, and absent means a circle predating the field whose creator still holds it. Never collapse those last two: absent reading as vacant would strip the controls from every existing circle |
...
| `utils/circleActivities.js` | The module registry that keeps `utils/circles.js` generic — `register(key, module)`, hooks incl. `snapshotExtras`/`participation`, and the `nominateFirst` capability |
| `utils/plans.js` | What a plan permits, and the only place the numbers live. Two plans (BUSINESS.md §5): `free` holds 3 circles, `host` is uncapped, admin uncapped as an operator's exemption. The limit counts **seats held** (`hostOf`), so leaving frees a slot without ending anybody's circle. `planOf()` treats an absent value as free — never query `{ plan: 'free' }`, which matches no account written before the field |
| `routes/circles.js` | `/api/circles` — the activity-agnostic surface (snapshot, my circles, join), the generic seed verbs (post/support/advance) and the gather verbs (respond/react); Threshold's own verbs stay on `/api/threshold`. **Circle creation and the host seat live here since 2026-09-16**: `POST /` makes a `gather` circle in DRAFT for any verified account, limited by plan (`utils/plans.js` — Free holds 3 seats, Host uncapped, admin uncapped); `POST /:id/start` is draft → running; and the three ways out are **`POST /:id/leave`** (any member — the circle stays, your seat empties), **`POST /:id/close`** (host, terminal, D29) and **`DELETE /:id`** (host, refused once anything is in it). `POST /:id/host` takes a vacant seat, and is the one place the plan limit is met besides creation — the funnel has no User and no opinion about billing. P15 rung 2 and Q6 had put hosting in the platform admin, MO-only; that surface was never built, so Threshold's `/new` — which hardcodes `activity: 'threshold'` — was the only creation path that existed |
| `utils/synthesisActivity.js` | Synthesis as a circle activity (2026-08-20, replacing `utils/circleSessions.js`): a seed with `activity: 'synthesis'` and `payload: { ideaId }`. Declares `nominateFirst`, so its seeds are born `nominated` — shared with the circle and contributable, but outside the queue until someone other than the author supports them |
| `utils/gather.js` | The builder's single-round activity (code/docs/plan/PRIMITIVES.md §9): prompt → responses → reveal, shapes story/placement/story+placement/words, sealed-or-open reveal, reactions, on-read aggregate. Registers itself as activity `gather`; write funnel for the primitive collections |
