const UnisonEmbedding = require('../models/UnisonEmbedding');

// The embedding index for the collective LLM (PLAN §6). Mirrors the funnel
// pattern of utils/unisonNodes.js: every function takes an injectable `store`
...
const mongoStore = {
  async upsert(doc) {
    return UnisonEmbedding.findOneAndUpdate(
  ...
  async remove({ instanceId, kind, refId }) {
    return UnisonEmbedding.deleteOne({ instanceId, kind, refId });
  },
  async removeByNode({ instanceId, nodeId }) {
    return UnisonEmbedding.deleteMany({ instanceId, nodeId });
  },
  async list(instanceId) {
    return UnisonEmbedding.find({ instanceId });
  },
};
