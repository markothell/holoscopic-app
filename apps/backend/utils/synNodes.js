...
// in one place. Per-idea room `syn:<instanceId>` (see sockets/synthesis.js).
let io = null;
function setIO(ioInstance) { io = ioInstance; }
function emitToIdea(instanceId, event, payload) {
  if (io && instanceId) io.to(`syn:${instanceId}`).emit(event, payload);
}
...
function fireIndex(method, ...args) {
  ...
  Promise.resolve()
    ...
    .catch(err => console.error('[synthesis-index]', method, err && err.message));
}

// ── Store: default Mongoose-backed implementation ───────────────────────────
const mongoStore = {
  async getNode(id) {
    return SynNode.findOne({ id });
  },
  async insertNode(fields) {
    return SynNode.create(fields);
  },
  async saveNode(node) {
    return node.save();
  },
  async findFrames(instanceId) {
    return SynFrame.find({ instanceId });
  },
  async getFrame(id, instanceId) {
    return SynFrame.findOne({ id, instanceId });
  },
  async createFrame(fields) {
    return SynFrame.create(fields);
  },
  // ── M1 (networking) reads ──────────────────────────────────────────────
  ...
  async findOwnerTopicHubs(instanceId, ownerId) {
    return SynNode.find({ instanceId, ownerId, kind: 'topic' });
  },
...
// The idea's title, seeded as the centre of a collaborator's map the moment
// they draft or join it (utils/synIdeas.js calls this). A topic-kind root
// carrying `isHome`, so every thought a collaborator adds hangs beneath the
// idea itself and the map reads as one thought space rather than a scatter of
// unrelated roots.
//
// Idempotent by design: joining is a no-op for an existing collaborator, and a
// second hub would give the map two centres, so an existing home hub is
// returned untouched. Callers fire this after the membership write, and it is
// deliberately safe to call twice.
async function seedHomeHub({ store = mongoStore, instanceId, ownerId, ownerHandle, title }) {
  if (!instanceId) throw new Error('instanceId is required');
  if (!ownerId || !ownerHandle) throw new Error('owner is required');
  // The home hub is itself a topic hub, so the existing owner-hub read covers
  // the uniqueness check without a new store method (an owner has a handful of
  // hubs, never a scan worth optimizing).
  const hubs = await store.findOwnerTopicHubs(instanceId, ownerId);
  const already = (hubs || []).find(n => n.isHome);
  if (already) return already;
  return createNode({
    store, instanceId, ownerId, ownerHandle,
    kind: 'topic',
    content: { topic: String(title || '').trim() || 'Untitled idea' },
    parentIds: [], edgeKind: 'root', isHome: true,
  });
}
...
// A marriage: the one structural break from a tree. Mints a NEW join node
...
// Ownership is the caller's concern (routes/synthesis.js checks node.ownerId
...
// Ownership is the caller's concern (routes/synthesis.js's loadOwnedNode 404s a
...
//      writer), and
//   2. a BORROWED SynNode on the RESPONDER'S OWN map, structure-mirrored
...
async function respond({
  ...
  emitToIdea(instanceId, 'reply_upserted', { postId: source.id, reply: entries.toClient(reply) });
