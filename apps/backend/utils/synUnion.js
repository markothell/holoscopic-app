...
const SynUnion = require('../models/SynUnion');
const indexUtil = require('./synIndex');

// The union orchestrator (UNION.md). Replaces the M3 "Ask
...
// helpers from synthesisChat.js verbatim (UNION.md — "relocate, don't just
// delete"):
...
function anchorUrlFor(hit) {
  if (hit.kind === 'reply') {
    return `/synthesis/n/${hit.nodeId}#reply-${hit.refId}`;
  }
  return `/synthesis/n/${hit.nodeId}`;
}

// Citations are assembled from the SELECTION set, in selection order, deduped
// by (kind, refId). Shape (UNION.md §6, unchanged from M3):
...
const positionalMongoStore = {
  ...
  async getFrames(ids) {
    ...
    return SynFrame.find({ id: { $in: ids } });
  },
};
...
// preserve it) — axis 0 is always x, axis 1 is always y (UNION.md §5 /
...
// Store-injected like the rest of the app's funnels (utils/synNodes.js,
// utils/synIndex.js) so the caching + staleness logic is unit-testable
...
const cacheMongoStore = {
  async findOne(instanceId) {
    return SynUnion.findOne({ instanceId });
  },
  async getOrCreate(instanceId) {
    return SynUnion.findOneAndUpdate(
      { instanceId },
