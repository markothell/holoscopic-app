...
// at all (lib/demo.ts).
//
// `readOnly` is the only thing the demo adds. It withholds the controls that
// would write — the compose sheet, the edit door, the facilitator's reveal,
// reactions — and touches nothing else: reading, tapping a seat, opening a
// chart, filtering by a word all behave exactly as they do for a member. It
// defaults to false, so a real circle sees the surface it has always seen.
...
export function GatherSurface({ circle, seed, extras, userId, header, onChanged, pageError, readOnly = false }: {
  circle: Circle;
  seed: Seed;
  ...
  /** Read-only: every control that would write is withheld rather than shown
   *  dead. Nothing else changes — the demo reads the same surface a member
   *  reads. */
  readOnly?: boolean;
}) {
  ...
  const canReact = !readOnly && Boolean(cfg.reactions) && storyShape && open;
  ...
  const stateLine = (() => {
    if (done) {
      // Read-only has no compose under it, so it must not offer one in words.
      if (readOnly) return 'Revealed — this is where the circle landed.';
      return myResponse
  ...
  const composeOpen = !readOnly && (composing || (!myResponse && !done) || (!myResponse && done));
  ...
  return (
    <Page>
      ...
      <div className="mt-3 flex flex-wrap items-baseline gap-x-4 gap-y-1">
        ...
        {!readOnly && !done && isFacilitator && (confirmReveal ? (
      ...
      {composeOpen ? (
        ...
        <div className="mt-6">
          ...
          <Card>
            ...
            {!readOnly && (!done || (Boolean(cfg.editAfterClose) && shape !== 'words')) && (
              <div className="mt-2">
