  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-6 pb-10 pt-[max(3rem,env(safe-area-inset-top))]">
      <header className="rise-in">
        <p className="eyebrow" style={{ color: 'var(--own)' }}>One account, every holoscopic game</p>
        <h1 className="display mt-3 text-6xl">
          Join<br /><span style={{ color: 'var(--synthesis)' }}>the study</span>
        </h1>
      </header>
      ...
      <section className="rise-in mt-auto pt-12" style={{ animationDelay: '0.1s' }}>
        ...
        />
        <Button variant="own" className="mt-5" onClick={submit} disabled={busy}>
          {busy ? 'Creating…' : 'Create account'}
        </Button>
        {error && <p className="mt-3 text-sm" style={{ color: 'var(--live)' }}>{error}</p>}
        <p className="mt-6 text-center text-sm text-mist-soft">
          Already have one?{' '}
          <Link className="underline" style={{ color: 'var(--own)' }} href={`/login?next=${encodeURIComponent(next)}`}>
            Sign in
          </Link>
        </p>
      </section>
