...
// "The Group" — the community synthesis view (S1, SYNTHESIS.md). Replaces
...
//
// Real path: GET /unison/synthesis on open (last cached result + stale
// badge), then POST /unison/synthesis {depth} streams a (re)generation — the
// same hand-parsed `text/event-stream` contract the old chat used for /chat
// (EventSource can't POST; see routes/unison.js's STREAMING CONTRACT
...
export default function SynthesisOverlay({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    UnisonService.getSynthesis(instanceId, userId)
      ...
      .catch(err => console.debug('[unison] synthesis load failed', err))
  ...
  function generate(depth: SynthesisDepth) {
    ...
    // The stale badge reflects the BRIEF artifact (routes/unison.js's
    ...
    UnisonService.synthesizeStream(instanceId, depth, userId, ac.signal)
  ...
  return (
    <OverlayShell title="The Group" eyebrow="Community synthesis" onClose={onClose}>
      {loading ? (
