...
// the mock feed client-side; real path (M1) reads GET /unison/feed —
// published UnisonNodes scoped to the community instance, each carrying
...
export default function FeedOverlay({
  ...
  const [feed, setFeed] = useState<UnisonNode[]>(useMock ? MOCK_FEED : []);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    UnisonService.feed(instanceId, userId)
      .then(({ feed: serverFeed }) => { if (!cancelled) setFeed(serverFeed); })
      .catch(err => console.debug('[unison] feed load failed', err))
  ...
  useEffect(() => {
    if (useMock) return;
    return unisonSocket.on('node_published', payload => {
      const { node } = payload as { node: UnisonNode };
