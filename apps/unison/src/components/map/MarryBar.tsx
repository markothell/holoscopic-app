...
export default function MarryBar({
  ...
  const label = (n: UnisonNode) => n.kind === 'topic' ? n.content.topic : n.content.thought;
  ...
  return (
    <div className="tone-in fixed inset-x-0 bottom-24 z-30 mx-auto flex w-[calc(100%-2.5rem)] max-w-sm items-center justify-between gap-3 rounded-2xl border px-4 py-3"
      style={{ background: 'var(--dusk-raised)', borderColor: 'var(--synthesis)', boxShadow: 'var(--shadow-card)' }}
    >
      ...
      <div className="flex shrink-0 gap-2">
        ...
        <button
          ...
          style={{ background: 'var(--synthesis)', color: 'var(--dusk-deep)' }}
        >
          Marry
        </button>
      </div>
