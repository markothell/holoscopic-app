import type { Community, UnisonFrame, UnisonNode, UnisonReply } from './types';

// Seed/mock data so the app renders and is fully interactive with no live
// backend (routes/unison.js is being built in parallel). useMyMap seeds from
...
// as-is; `isHome` is the only thing that marks it as the root (types.ts).
...
export const MOCK_FRAMES: UnisonFrame[] = [
  {
    id: 'fr_depth',
    instanceId: MOCK_COMMUNITY.id,
    parentInstanceId: 'unison',
    poleA: 'considered',
    poleB: 'instinctive',
    key: null,
...
function node(partial: Partial<UnisonNode> & Pick<UnisonNode, 'id' | 'kind' | 'content'>): UnisonNode {
  return {
    instanceId: MOCK_COMMUNITY.id,
...
export const MOCK_NODES: UnisonNode[] = [
  node({
    id: MOCK_HOME_NODE_ID,
    kind: 'topic',
    content: { topic: MOCK_USER_HANDLE, thought: '', context: '' },
    edgeKind: 'root',
    isHome: true,
  }),
  node({
...
export const MOCK_POST: UnisonNode = node({
  id: 'n_remote_ritual',
  kind: 'thought',
  content: {
    topic: '',
...
export const MOCK_REPLIES: UnisonReply[] = [
  {
...
// mirrors GET /unison/feed's server-side enrichment (utils/unisonNodes.js
// #feed) so the mock and real paths render identically.
export const MOCK_FEED: UnisonNode[] = [
...
const MOCK_REMOTE_NODES: UnisonNode[] = (() => {
  const byId = new Map<string, UnisonNode>();
  for (const n of [MOCK_POST, ...MOCK_FEED]) if (!byId.has(n.id)) byId.set(n.id, n);
  return [...byId.values()];
})();

export function resolveMockRemoteNode(id: string): UnisonNode | undefined {
